...
do
    ...
    awk 'BEGIN{printf "%-10s %10.4f %8.2f%% %10.4f %8.2f%% %10.4f %8.2f%% %10d %10.2f%%\n", \
        "'$benchmark'", \
        '$myopt_latency', '$myopt_latency'/'$tsrouter_latency'*100, \
        '$myopt_ipc', '$myopt_ipc'/'$tsrouter_ipc'*100, \
        '$myopt_stdev', '$myopt_stdev'/'$tsrouter_stdev'*100, \
        '$myopt_max', '$myopt_max'/'$tsrouter_max'*100}'
done
